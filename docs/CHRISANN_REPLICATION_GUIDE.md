...
## 15. Native AgentMail Email Integration (CONFIG ONLY)

**What Tim has:**
Tim's agent uses AgentMail's native IMAP/SMTP endpoints via Hermes Agent's built-in platform adapter (`plugins/platforms/email/adapter.py`).

```env
EMAIL_ADDRESS=bucky-1840@agentmail.to
EMAIL_PASSWORD=<agentmail-api-key>
EMAIL_IMAP_HOST=imap.agentmail.to
EMAIL_IMAP_PORT=993
EMAIL_SMTP_HOST=smtp.agentmail.to
EMAIL_SMTP_PORT=587
EMAIL_ALLOWED_USERS=villagertim@gmail.com
EMAIL_HOME_ADDRESS=villagertim@gmail.com
EMAIL_POLL_INTERVAL=15
EMAIL_TRUST_FROM_HEADER=true
```

**Steps for Chrisann:**
1. Create an AgentMail inbox for Chrisann (e.g. `chrisann-agent@agentmail.to`) in the AgentMail dashboard.
2. Obtain Chrisann's AgentMail API key (`am_us_...`).
3. Add the following to `.chrisann-agent.env` and `data/chrisann/hermes/.env`:
   ```env
   EMAIL_ADDRESS=<chrisann-inbox-id>@agentmail.to
   EMAIL_PASSWORD=<chrisann-agentmail-api-key>
   EMAIL_IMAP_HOST=imap.agentmail.to
   EMAIL_IMAP_PORT=993
   EMAIL_SMTP_HOST=smtp.agentmail.to
   EMAIL_SMTP_PORT=587
   EMAIL_ALLOWED_USERS=<chrisann-human-email>
   EMAIL_HOME_ADDRESS=<chrisann-human-email>
   EMAIL_POLL_INTERVAL=15
   EMAIL_TRUST_FROM_HEADER=true
   ```
4. Restart Chrisann's agent container:
   ```bash
   docker compose restart agent-chrisann
   ```
5. Verify:
   ```bash
   docker logs hermes-chrisann --tail 50 | grep -i email
   ```
   Should output: `[Email] Connected as <chrisann-inbox-id>@agentmail.to`
