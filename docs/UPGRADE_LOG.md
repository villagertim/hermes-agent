...
## Upgrade: v0.16.0 → v0.18.0 (v2026.7.1)

**Date**: 2026-07-04
**Target**: tag `v2026.7.1`
**Method**: backup-reset-restore with dialectical Red/Blue teaming
**Conversation**: `2204ef71-7e1b-421c-be8c-db67996360e0`

### Pre-Upgrade State
- Running version: `v0.16.0` (tag `v2026.6.5`)
- Custom files: Two strict tenant profiles (`data/tim`, `data/chrisann`) bridged to LiteLLM reverse-proxies via internal docker networks. Nginx proxy routing for `villagertim.com` / `villagerchrisann.com`.
- Hardened constraints: Strict separation of environments, all agent execution must be driven by `local-model` via `litellm`.

### Plan
A rigorous 13-step plan generated via Red-Teaming (Devil's Advocate) and Blue-Teaming (Defender). We bypassed standard `git pull` in favor of a hard backup, branch reset, and selective restoration to prevent `.gitignore` tracking conflicts for sensitive files.

### What Changed Upstream (v0.16.0 → v0.18.0)
- **Dockerfile**: Switched to `debian:13.4` (Trixie).
- **Dashboard**: Deprecated `--insecure` flag. Binding to `0.0.0.0` now explicitly requires an auth provider configured in `config.yaml` (`dashboard.basic_auth` or `dashboard.oauth`).

### Mitigations & Adjustments
1. **Chromium Compatibility**: Checked Debian 13.4 repos for Chromium support; `chromium` package was successfully injected back into the Dockerfile.
2. **Docker Compose**: Confirmed topology paths did not drift. Added `HERMES_UID` / `HERMES_GID` explicitly to the environment to ensure volume permissions remained valid.
3. **Dashboard Auth Fix**: The dashboards crashed in a restart loop due to the missing auth provider when bound to `0.0.0.0`. Fixed by generating scrypt password hashes and populating `dashboard.basic_auth` in both `data/tim/hermes/config.yaml` and `data/chrisann/hermes/config.yaml`.
4. **.gitignore**: Explicitly appended our multi-tenant custom config rules (`data/tim`, `data/chrisann`, `.env`, `.chrisann-agent.env`, etc.) to prevent committing secrets to the upstream-sync branch.

### Verification
- [x] Containers built and healthy (`docker compose ps`)
- [x] Python dependencies verified (`uv lock --check`)
- [x] SQLite database integrity check (`PRAGMA integrity_check`)
- [x] Nginx isolation (401 Unauthorized block on external dashboard access)
- [x] Model routing via LiteLLM for both Tim's models (`cheap`, `reasoning`) and Chrisann's models (`chrisann-cheap`)
- [x] Dashboards successfully bound to `0.0.0.0` with `basic_auth` and loaded correctly.

### Lessons
- The new v0.18.0 dashboard security hardening requires explicit authentication when exposing ports over non-loopback interfaces. The `--insecure` flag is entirely deprecated. For multi-tenant setups behind reverse proxies, `basic_auth` must be configured inside Hermes itself.
- Moving to `debian:13.4` required no major changes, but checking package availability (like `chromium`) beforehand via apt-cache was crucial.
- Using a Red/Blue team planning approach avoided breaking the multi-tenant architecture by enforcing strict backup and manual file restoration.
