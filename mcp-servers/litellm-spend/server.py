...
def get_api_key() -> str:
    # Check LITELLM_VIRTUAL_KEY first, fallback to OPENROUTER_API_KEY (framework legacy name for LiteLLM virtual key)
    return os.environ.get("LITELLM_VIRTUAL_KEY") or os.environ.get("OPENROUTER_API_KEY", "")

@mcp.tool()
async def get_spend_summary() -> str:
    """Retrieve the current agent's spend summary and budget limits from LiteLLM proxy."""
    api_key = get_api_key()
    if not api_key:
        return "Error: Agent LiteLLM Virtual Key (LITELLM_VIRTUAL_KEY / OPENROUTER_API_KEY) is not set in environment."
